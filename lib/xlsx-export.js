...
(function (global) {
  ...
  // 16 细边框-表头首行(粗体+浅灰底+换行) | 17 细边框-表头次行(浅灰底) | 18 细边框-文本居中
  // 19 细边框-数字3位 | 20 细边框-加粗文本居中(测点)
  var STYLES_XML = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n' +
    ...
    '<cellXfs count="21">' +
      ...
      '<xf numFmtId="0" fontId="1" fillId="13" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf>' +
  ...
  function sheetXml(sheet) {
    ...
    sheet.rows.forEach(function (row, ri) {
      ...
      var ht = (sheet.rowHeights && sheet.rowHeights[ri + 1]) ? (' ht="' + sheet.rowHeights[ri + 1] + '" customHeight="1"') : '';
      body += '<row r="' + (ri + 1) + '"' + ht + '>' + cells + '</row>';
    });
    var colsXml = '';
    if (sheet.cols) {
